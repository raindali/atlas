...
public class ExtensionLoader<T> {
        ...
        for (Field field : fields){
            ...
            if (fieldType.isInterface() && withSpiAnnotation(fieldType)) {
                ...
                if(field.isAnnotationPresent(Resource.class)){  //通过注解注入
                    ...
                    Method method = ReflectionUtils.findMethod(clz, name, field.getType());
                    if(method!=null){
                        try {
    ...
    private synchronized void loadExtensionClasses() {
        ...
        this.extensionClasses = loadExtensionClasses0(PREFIX);
        this.singletonInstances = new ConcurrentHashMap<>();
        initialized = true;
    }

    private synchronized ConcurrentMap<String, Class<T>> loadExtensionClasses0(String prefix) {
